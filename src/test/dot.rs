...
mod test {
	...
	fn dot() {
		...
		let v7 = Vector::from(vec![Complex{re: 1.0, im: 4.0}, Complex{re: -6., im: 3.}]);
		let v8 = Vector::from(vec![Complex{re: 5., im: 2.}, Complex{re: 2.0, im: 5.0}]);

		assert_eq!(0.0, v1.dot(&v2));
		assert_eq!(2.0, v2.dot(&v2));
		assert_eq!(9.0, v3.dot(&v4));
		assert_eq!(9, v5.dot(&v6));
		assert_eq!(Complex{re: 16., im: 54.}, v7.dot_complex(&v8));
	}
}